...
BootScene
  ...
  -> Boss CombatScene
  -> SummaryScene
  -> TimelineReport or MainMenuScene
```

实现建议：

- `BootScene` 负责加载资源和读取存档。
- `MainMenuScene` 负责开始游戏、展示本地存档摘要和进入后续菜单。
- `MapScene` 展示时间树和可选节点。
...
- `src/game/scenes/MainMenuScene.ts` 是游戏首屏。
- `MainMenuScene` 读取 `loadSaveData()`，展示 active residues、最近一局、最近腐化和历史最高腐化。
- `Start Run` 调用 `startNewRun()` 后进入 `MapScene`。
- `Reset Save` 调用 `clearSaveData()` 清空项目本地存档。
- `SummaryScene` 提供返回 `MainMenuScene` 的按钮。
- Vite dev 环境下 `src/main.ts` 暴露 `window.__chronoEchoGame`，用于 Playwright 验证 active scene；生产构建不依赖该句柄。
